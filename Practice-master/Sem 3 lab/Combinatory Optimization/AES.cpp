...
{
    int idx;
    for (idx = 0; idx <= 15; idx++) modifiedKey[idx]=originalKey[idx]; 
...
int main()
{
    ...
    int txtLen=sizeof(inputText)-1;
    int extendedLen;

    if((txtLen%16)!=0)
        extendedLen=txtLen+(16-(txtLen%16));

    else extendedLen=txtLen;
    ...
    {
        unsigned char temp[16];
		for(int j=0;j<16;j++) temp[j]=encryptedTxt[i+j];
		encryption(temp);
		for(int j=0;j<16;j++) encryptedTxt[i+j]=temp[j];
	}
